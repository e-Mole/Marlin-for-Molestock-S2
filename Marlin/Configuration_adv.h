 ...
 */
#define E0_AUTO_FAN_PIN 11 //RAMPS FAN EXTENDER pin D11 (Molestock S2 heatsink fan + LED light, cyclops + chimera) or D9 (cyclops only)//TFs mod
#define E1_AUTO_FAN_PIN 11 //RAMPS FAN EXTENDER pin D11 (Molestock S2 heatsink fan + LED light, cyclops + chimera) or D9 (cyclops only)//TFs mod
...
//#define HOMING_BACKOFF_MM { 2, 2, 2 }  // (mm) Move away from the endstops after homing //TFs: not for delta now :-(
...
#define LCD_TIMEOUT_TO_STATUS 300000 //5 min
...
// TFs mod: If enable, then KILL BUTTON run only gcode_M125 for pause (only for testing purpose, not "production" use!!!)
//#define NO_KILL
